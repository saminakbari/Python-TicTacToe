...
def write_game_result(game, result):
    content = "Player 1: " + game.player_names[0]
    content += "\nPlayer 2: " + game.player_names[1]
    content += "\nResult: " + str(result)
    content += "\n------------------------------------\n"
...
def show_history(username):
    ...
    for i in range(len(lines) // 4):
        if re.search(pattern, lines[i]) or re.search(pattern, lines[i + 1]):
            content += lines[i] + lines[i + 1] + lines[i + 2] + lines[i + 3]
